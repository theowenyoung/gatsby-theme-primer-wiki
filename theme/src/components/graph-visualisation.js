...
const AnimatedBox = styled(animated.div)`
  ...
  box-shadow: ${themeGet("shadows.overlay.shadow")};
`;
const AnimateOverlay = styled(animated.div)`
  z-index: 98;
  position: fixed;
  top: 0;
  right: 0;
  ...
  height: 100%;
  width: 100%;
  background-color: ${themeGet("colors.fade.black10")};
...
const GraphVisualisation = ({ setGraphState, graphState, tagsGroups }) => {
  const [nodesData, linksData] = useGraphData(tagsGroups);
  ...
  const [modalSpring, api] = useSpring(() => ({
    to: {
      bottom: "0%",
      right: "0%",
  ...
  }, [graphState, api]); // eslint-disable-line react-hooks/exhaustive-deps
